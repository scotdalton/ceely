  ...
  module Interval
    describe Pythagorean do
      pythagoreans.each do |index, expected_values|
        ...
        context "when it's the #{index} pythagorean interval," do
          subject(:pythagorean) { Pythagorean.new(Note.new(528.0), index) }

          describe '#index' do
            it 'has the index that we expect' do
              expect(pythagorean.index).to eq(index)
            end
          end

          describe '#frequency' do
            it 'has the frequency that we expect' do
              expect(pythagorean.frequency).to eq(expected_frequency)
            end
          end

          describe '#factor' do
            it 'has the factor we expect' do
              expect(pythagorean.factor).to be_a(Rational)
              expect(pythagorean.factor).to eq(Rational(expected_factor_num, expected_factor_den))
              expect("#{pythagorean.factor}").to eq(expected_factor)
            end
          end

          describe '#note' do
            it 'has the note that we expect' do
              expect(pythagorean.note).to eq(Note.new expected_frequency)
            end
          end

          describe '#octave' do
            it 'is the octave we expect' do
              expect(pythagorean.octave).to eq(expected_octave)
            end
          end

          describe '#octave_adjusted_denominator' do
            it 'is the octave denominator we expect' do
              expect(pythagorean.octave_adjusted_denominator).to eq(expected_octave_adjusted_denominator)
            end
          end

          describe '#octave_adjusted_factor' do
            it 'has the octave adjusted factor we expect' do
              expect(pythagorean.octave_adjusted_factor.to_f).to eq(expected_octave_adjusted_factor)
              # expect("#{pythagorean.octave_adjusted_factor}").to eq(expected_octave_adjusted_factor)
            end
          end

          describe '#octave_adjusted_frequency' do
            it 'has the octave_adjusted_frequency that we expect' do
              expect(pythagorean.octave_adjusted_frequency).to eq(expected_octave_adjusted_frequency)
            end
          end

          describe '#octave_adjusted_note' do
            it 'has the octave_adjusted_note that we expect' do
              expect(pythagorean.octave_adjusted_note).to eq(Note.new expected_octave_adjusted_frequency)
            end
          end

          # describe '#to_s' do
          #   it 'goes to string as we expect' do
          #     expect(pythagorean.to_s).to eql(%Q{
          #       Index: #{index}
